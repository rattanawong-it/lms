...
| `(admin)` | `/admin`, `/admin/{users,departments,categories,courses,certificates,reviews,announcements,reports,screen-events,audit,settings,deletion-requests,score-curve,coupons}` | DEPT_ADMIN+ (บางหน้าเฉพาะ SUPER_ADMIN — audit/settings/deletion-requests/coupons เป็น SUPER_ADMIN) |
